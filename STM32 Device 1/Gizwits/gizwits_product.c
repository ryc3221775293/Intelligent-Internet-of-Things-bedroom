...
u8 AUDIO_SECOPEN[6] = {0xAA, 0x07, 0x02, 0x00, 0x0A, 0xBD};
u8 AUDIO_SECLOSE[6] = {0xAA, 0x07, 0x02, 0x00, 0x0B, 0xBE};
u8 AUDIO_CLOSELIGHT[6] = {0xAA, 0x07, 0x02, 0x00, 0x03, 0xB6};
u8 AUDIO_OPENLIGHT[6] = {0xAA, 0x07, 0x02, 0x00, 0x04, 0xB7};
u8 AUDIO_OPENFAN[6] = {0xAA, 0x07, 0x02, 0x00, 0x0C, 0xBF};
u8 AUDIO_CLOSEFAN[6] = {0xAA, 0x07, 0x02, 0x00, 0x0D, 0xC0};
/**@} */
  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      ...
      case EVENT_LED:
        ...
        break;
      case EVENT_FirstDoor:
        currentDataPoint.valueFirstDoor = dataPointPtr->valueFirstDoor;
        GIZWITS_LOG("Evt: EVENT_FirstDoor %d \n", currentDataPoint.valueFirstDoor);
        if(0x01 == currentDataPoint.valueFirstDoor)
        {
          //user handle
					Open();
					...
					delay_ms(1000);
					Close();
					currentDataPoint.valueFirstDoor=0x00;
        }
        else
        {
          //user handle 
        ...
        break;
      case EVENT_SecondDoor:
        currentDataPoint.valueSecondDoor = dataPointPtr->valueSecondDoor;
        GIZWITS_LOG("Evt: EVENT_SecondDoor %d \n", currentDataPoint.valueSecondDoor);
        if(0x01 == currentDataPoint.valueSecondDoor)
        {
          //user handle
        ...
        {
          //user handle  
					Motorccw_angle(90,500);
					AUDIO_PLAY(AUDIO_SECLOSE);
        }
        break;
