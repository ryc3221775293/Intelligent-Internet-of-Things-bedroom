...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagFirstDoor)
    {
        info->event[info->num] = EVENT_FirstDoor;
        info->num++;
        dataPoints->valueFirstDoor = gizStandardDecompressionValue(FirstDoor_BYTEOFFSET,FirstDoor_BITOFFSET,FirstDoor_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSecondDoor)
    {
        info->event[info->num] = EVENT_SecondDoor;
        info->num++;
        dataPoints->valueSecondDoor = gizStandardDecompressionValue(SecondDoor_BYTEOFFSET,SecondDoor_BITOFFSET,SecondDoor_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueFirstDoor != cur->valueFirstDoor)
    {
        GIZWITS_LOG("valueFirstDoor Changed\n");
        ret = 1;
    }
    if(last->valueSecondDoor != cur->valueSecondDoor)
    {
        GIZWITS_LOG("valueSecondDoor Changed\n");
        ret = 1;
    }
    ...
    if(last->valueAir != cur->valueAir)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueAir Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(FirstDoor_BYTEOFFSET,FirstDoor_BITOFFSET,FirstDoor_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFirstDoor);
    gizStandardCompressValue(SecondDoor_BYTEOFFSET,SecondDoor_BITOFFSET,SecondDoor_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSecondDoor);
    ...
    devStatusPtr->valueAir = gizY2X(Air_RATIO,  Air_ADDITION, dataPoints->valueAir); 
